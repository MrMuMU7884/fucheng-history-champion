...
function render(){
  ...
  $('actionButton').hidden=false;$('actionButton').disabled=true;$('nextButton').hidden=true;
...
function choose(button){if(revealed)return;document.querySelectorAll('.answer').forEach(b=>b.classList.remove('chosen'));button.classList.add('chosen');selected=button.dataset.answer;$('actionButton').disabled=false;}
function reveal(){if(revealed||!selected)return;revealed=true;attempted++;const item=questions[index];const win=selected===item.answer;if(win)correct++;document.querySelectorAll('.answer').forEach(b=>{b.disabled=true;if(b.dataset.answer===item.answer)b.classList.add('correct');if(b.dataset.answer===selected&&!win)b.classList.add('wrong');});$('result').textContent=win?'恭喜🎉，您答对了':'别灰心，再试一次！';$('result').className=`result ${win?'good':'bad'}`;$('score').textContent=`得分：${correct} / ${attempted}`;$('actionButton').hidden=true;$('nextButton').hidden=false;}
...
document.addEventListener('keydown',e=>{if(e.code==='Space'&&!['BUTTON'].includes(document.activeElement.tagName)){e.preventDefault();revealed?next():reveal();}});$('actionButton').onclick=reveal;$('nextButton').onclick=next;restart();
